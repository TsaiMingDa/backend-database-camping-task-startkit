...
set status = '已取消', cancelled_at = '2024-11-24 17:00:00'
...
select "USER".name, "COACH".experience_years, "SKILL"."name"  from "COACH_LINK_SKILL"
...
select "SKILL".name, count(*) as couch_total from "COACH_LINK_SKILL"
inner join "SKILL" on "SKILL".id = "COACH_LINK_SKILL".skill_id
group by "SKILL".name
order by couch_total desc
limit 1;
-- 6-3. 查詢：計算 12 月份組合包方案的銷售數量
-- 顯示須包含以下欄位： 組合包方案名稱, 銷售數量
select "CREDIT_PACKAGE".name, count(*) as 數量 from "CREDIT_PURCHASE"
